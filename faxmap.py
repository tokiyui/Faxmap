...
# 平滑化
dsp['mslp'].data = denoise_bilateral(dsp['mslp'].data.magnitude, sigma_color=3, sigma_spatial=5, channel_axis=None) * dsp['mslp'].data.units
...
for tagHp in [300, 400, 500, 700, 850, 925]:
    ...
    ttds = np.asarray(ttds, dtype=float)
    u_winds = np.asarray(u_winds, dtype=float)
    v_winds = np.asarray(v_winds, dtype=float)

    mask = (np.isfinite(temps) & np.isfinite(ttds) & np.isfinite(u_winds) & np.isfinite(v_winds))
    lons = np.asarray(lons)[mask]
    lats = np.asarray(lats)[mask]
    temps = temps[mask]
    ttds = ttds[mask]
    u_winds = u_winds[mask]
    v_winds = v_winds[mask]

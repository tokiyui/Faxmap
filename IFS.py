...
# 8時間引く
adjusted_time = now_utc - datetime.timedelta(hours=7)
...
for ft in fts: 
    ...
    fname_gfs = gsm_fn_t.format(i_year,i_month,i_day,i_hourZ,ft,name)
    ...
    valTp, _, _ = grbs.select(name='Total precipitation rate')[0].data(lat1=latS,lat2=latN,lon1=lonW,lon2=lonE)
    ...
    # 出力先ディレクトリを作成
    output_dir = os.path.join("IFS/", dt_str)

...
fts = list(range(0, 144, 3))

# データの格納先フォルダー名
##!!! GRIB2データの保存先をFolderを指定すること !!!
data_fld= "https://nomads.ncep.noaa.gov/cgi-bin/filter_gfs_0p25.pl?dir=%2Fgfs.{0:04d}{1:02d}{2:02d}%2F{3:02d}%2Fatmos&file=gfs.t{4:02d}z.pgrb2.0p25.f{5:03d}&var_ABSV=on&var_CPRAT=on&var_HGT=on&var_PRATE=on&var_PRMSL=on&var_RH=on&var_SPFH=on&var_TMP=on&var_UGRD=on&var_VGRD=on&var_VVEL=on&lev_2_m_above_ground=on&lev_10_m_above_ground=on&lev_925_mb=on&lev_850_mb=on&lev_700_mb=on&lev_500_mb=on&lev_400_mb=on&lev_300_mb=on&lev_surface=on&lev_mean_sea_level=on&subregion=&toplat=60&leftlon=90&rightlon=180&bottomlat=0"

for ft in fts: 
    # 読み込むGRIB2形式GSMのファイル名
    dat_fld=data_fld.format(i_year,i_month,i_day,i_hourZ,i_hourZ,ft)
    gsm_fn_t="gfs.t{0:02d}z.pgrb2.0p25.f{1:03d}"
    fname_gfs = gsm_fn_t.format(i_hourZ,ft)
 
    # HTTPでファイルダウンロード
    response = requests.get(dat_fld)
    ...
    valSlp, _, _ = grbs.select(name='Pressure reduced to MSL')[0].data(lat1=latS,lat2=latN,lon1=lonW,lon2=lonE)
    ...
    # 気象庁RGBカラー
    jmacolors=np.array([[242,242,242,1], [160,210,255,1], [33,140,255,1], [0,65,255,1], [250,245,0,1], [255,153,0,1], [255,40,0,1], [180,0,104,1]],dtype=float)
    jmacolors[:,:3] /=256
    rainlevels=[0,1,5,10,20,30,50,80] #等高線値
    ...
    # 出力先ディレクトリを作成
    output_dir = os.path.join("GFS/", dt_str)
